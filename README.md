...
- **Qwen `system` subtypes are classified by name, not by observed shape** —
  ...
  `at_command` → `context_injection`, and so on) is read off Qwen's own
  `KNOWN_RECORD_SUBTYPES` list rather than off a transcript that exercised
  them. Fields pass through verbatim either way, so the risk is a record
  filed under a slightly wrong kind, not lost or mangled content — but a
  subtype that turns out to be state while sitting in `activity` would be
  invisible to a consumer filtering on `session_state`.
