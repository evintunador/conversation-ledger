...
cledger is built on [annals](https://github.com/evintunador/annals), a
content-agnostic library for append-only records in git notes. annals owns
the storage mechanics — identity, dedup, commit anchoring, re-anchoring,
redaction-on-write, the pre-push secret scan, and transport — namespaced
per producer; cledger is its conversation vocabulary: the event kinds
below, the capture adapters, and the CLI views, bound to the
`refs/notes/conversation-ledger` namespace (`.cledger.json`,
`.git/conversation-ledger/`). Events use the annals envelope (`ev2-…` ids,
schema `annals/v1`, grouping field `stream` — a stream is a conversation
here), with cledger's vocabulary metadata (who spoke, which model served a
turn) carried in the envelope's `meta`. Other tools extend the ledger by
writing their own kinds through `appendEvents` — identified by
...
`--all`, which backfills every session that CLI scopes to the exact current
...
- **Codex sub-agent coverage: confirmed, and it was corrupting the parent**
  *(shipped, 0.24.0; discovery completed after 0.26.0)* — the question was whether codex writes a separate
  ...
  *Discovery is now complete.* Sub-agent activity carries an `agent_thread_id`
  (directly in older `event_msg/sub_agent_activity` payloads and under
  `item_completed.item` in current rollouts), and 5 of the 6 distinct values
  seen resolve to a rollout file on disk (the sixth was presumably pruned), so
  `agent_thread_id` → `rollout-*-<id>.jsonl` is a working trail — the codex
  analogue of claude-code's `subagents/` sibling directory. Capture walks that
  trail recursively across the Codex sessions tree, skips children whose files
  were pruned, and guards against cycles. A hook for the parent is therefore
  sufficient to capture every reachable child rollout still on disk.
- **The tail sweep is a workaround, not a fix** — a session cannot capture its
  ...
  install are deliberately never adopted by a hook. `cledger capture
  claude-code --all` now adopts them explicitly for the exact current
  directory, anchoring knowingly rather than silently. A
  `SessionEnd`-with-delay would still close the final-tail gap properly.
