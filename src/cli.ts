...
import { findRepo, forgeForRepo, runRecordsCommand, suggestMappings, type RepoInfo } from "annals";
import {
  appendEvents,
  manualReAnchor,
  NOTES_REF,
...
Usage:
  ...
  cledger sync [--remote R] [--push|--fetch] [--all|--rev R] [--no-scan] [--paranoid] [--report]
                                           original cledger sync options and branch scope
  cledger review [--paranoid] [--context N]  original interactive review (HUMAN ONLY)
  cledger inspect EVENT_ID [--context N] [--reveal] [--stdout]
                                           inspect one event into a private temp file (HUMAN ONLY)
  cledger redact EVENT_ID (--pattern REGEX|--all) [--reason TEXT]  HUMAN ONLY
  cledger allow FINGERPRINT... [--global]  HUMAN ONLY
  cledger re-anchor [--target R] [--apply] [--no-forge]
  cledger re-anchor OLD_REV... --onto REV  evidence-ranked suggestions / manual mapping
...
async function cmdSync(flags: Flags): Promise<void> {
  ...
  if (flags["no-scan"] === true && mode !== "fetch" && refuseAgent("sync --no-scan")) return;
...
function refuseAgent(command: string): boolean {
  const marker = inAgentSession();
  if (!marker) return false;
  process.stderr.write(
    `cledger ${command}: refusing to run inside a coding-agent session (saw $${marker}).\n` +
 ...
 * Refuses inside an agent session for exactly inspect's reason. It exists
 * for the human half of the review split; agents can use scan for counts.
 */
...
async function cmdReAnchor(positional: string[], flags: Flags): Promise<void> {
  if (positional.length > 0 && refuseAgent("re-anchor manual")) return;
  const repo = await requireRepo();
